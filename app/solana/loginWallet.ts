...
export async function loginWithWallet(
  ...
  try {
    ...
    // IMPORTANT: Use blockchain as source of truth for what has been claimed
    // Points > 100 means user already claimed wallet connect (20) + verify (40) bonuses
    // So ALWAYS mark achievements as claimed on re-login to prevent double counting
    // The wallet_connected and verified flags are set above, but achievements_claimed
    // must be set AFTER to prevent getBonusPoints() from adding them again
    localStorage.setItem('desocial_achievements_claimed', 'true');
    localStorage.setItem('desocial_referral_bonus_claimed', 'true');
    localStorage.removeItem('desocial_points_proof'); // Clear usage points - blockchain is source of truth
    ...
    console.log('✅ Login complete - blockchain is source of truth, achievements marked as claimed');
